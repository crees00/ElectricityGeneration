...
## Import gridwatch data and plot out in interactive line plot ###############

def nowtime():
    now=datetime.datetime.now()
    return "".join([str(i) for i in (now.day,0,now.month,now.year,now.hour,now.minute)])

data161 = getData.getGridwatchData()
...
def updateToolTips(dataLength=dataLength):
    '''Change the type and format of the tooltips that show on hovering
    the mouse over the lines. Also can adjust 'space' so that
    less data is shown on the graphs'''
    tooltips = []
    if dataLength == 'month':
...
ylist = [' '+z.lower() for z in zlist] # to coincide with df col names
showcols = ylist[:3]
dictOfLines={}
