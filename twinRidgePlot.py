...
print(len(years))
colour_grad = 8
lim1 = int(255 - (len(years) * colour_grad))
lim2 = int(colour_grad * len(years))
palette1 = [cc.coolwarm[i * colour_grad + lim1] for i in range(len(years) + 1)]
palette2 = [cc.coolwarm[lim2 - i * colour_grad] for i in range(len(years) + 1)]
...
for i, year in enumerate(reversed(years)):
    ...
    p.quad(top=y, bottom=((str(year), 0),) * len(y), left=edges[:-1], right=edges[1:],
           fill_color=palette1[len(years) - i], line_color=palette1[len(years) - i], alpha=0.5, legend='Nuclear')
    p.quad(top=y2, bottom=((str(year), 0),) * len(y2), left=edges[:-1], right=edges[1:],
           fill_color=palette2[len(years) - i], line_color=palette2[len(years) - i], alpha=0.5, legend='Wind')
